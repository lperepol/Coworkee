...
app.get(config.direct.apiUrl, function (req, res, next) {
    console.log("server->app.js->app.get_1");
    try {
        directApi.getAPI(
                function (api) {
...
app.get(config.direct.classRouteUrl, function (req, res, next) {
    console.log("server->app.js->app.get_2");
    var err = new Error('Internal Server Error');
...
app.post(config.direct.classRouteUrl, function (req, res) {
    console.log("server->app.js->app.post");
...
if (app.get('env') === 'development') {
  console.log("server->app.js->app.get");
...
app.use(function (err, req, res, next) {
    console.log("server->app.js->app.use");
    res.status(err.status || 500);
    res.render('error', {
        message: err.message,
        error: {}
